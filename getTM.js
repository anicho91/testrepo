  ...
  $.ajax({
    ...
    for (let i = 0; i < response._embedded.events.length; i++) {
      ...
      for (let j = 0; j < response._embedded.events[i]._embedded.venues.length; j++) {
        ...
        let venueCity =
          response._embedded.events[i]._embedded.venues[j].city.name;

        let venueCountry =
          response._embedded.events[i]._embedded.venues[j].country.name;
        let venueAddress =
          response._embedded.events[i]._embedded.venues[j].address.line1;

        const display = $("#display");
        const newDiv = $("<div>");

        if (
          response._embedded.events[i]._embedded.venues[j].country
            .countryCode == "US"
        ) {
          let venueState =
            response._embedded.events[i]._embedded.venues[j].state.name;
          newDiv.html(
            `<p><strong>${venueName}</strong> at ${venueAddress} ${venueCity}, ${venueState}, ${venueCountry}</p> <p><a href=${eventURL}>Link to Event</a></p>`
          );

          display.append(newDiv);
        } else {
          newDiv.html(
            `<p><strong>${venueName}</strong> at ${venueAddress} ${venueCity}, ${venueCountry}</p> <p><a href=${eventURL}>Link to Event</a></p>`
          );
